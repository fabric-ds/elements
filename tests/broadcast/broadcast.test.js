...
test('Basic broadcast with defaults', async () => {
  ...
  await wait(50);
  const broadcasts = document.querySelector('f-toast-container').renderRoot.querySelectorAll('f-toast');
  expect(broadcasts.length).to.equal(1);
});

test('Basic broadcast with defined URL and interval', async () => {
  const el = await fixture(html`<f-broadcast api="http://localhost:4053/single-broadcast" url="http://test" interval="5000"></f-broadcast>`);
  await wait(50);
